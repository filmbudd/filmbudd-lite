            ...
            if (code && parseInt(code) > 0) {
              ...
              if (work) {
                ...
                if (source === configs.SOURCE_CN) {
                  _work.imRating = work.im_rating?.split("/")[0] || MidLineEllipsis;
                  _work.imUrl = work.im ? `https://www.imdb.com/title/${work.im}/` : "";
                  // TODO: Change the link to feedback page if there is no linked record/404”
                } else if (source === configs.SOURCE_IM) {
                  _work.cnRating = work?.cn_rating?.split("/")[0] || MidLineEllipsis;
                  _work.cnUrl = work.cn ? `https://movie.douban.com/subject/${work.cn}/` : "";
                  // TODO: Change the link to feedback page if there is no linked record/404”
                }

                console.log(JSON.stringify(_work));
