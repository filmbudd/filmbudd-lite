...
function defaultLinksComposer(keyword: string): SearchLink[] {
  return [
    {
      name: SITE_NAME_DOUBAN,
...
export function installFeatureQuickSearchContextMenus(extensionName: string, linksComposer?: LinksComposer) {
  ...
  browser.contextMenus.onClicked.addListener(async (info: Menus.OnClickData, tab?: Tabs.Tab) => {
    ...
    const links = linksComposer ? linksComposer(keyword) : defaultLinksComposer(keyword);
